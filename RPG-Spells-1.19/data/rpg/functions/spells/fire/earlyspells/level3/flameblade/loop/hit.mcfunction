execute at @s run execute at @e[type=!armor_stand,type=!item,type=!item_frame,distance=1..5,tag=!summonedFireBase,tag=!Summoned] if block ~ ~ ~ minecraft:air run summon armor_stand ~ ~ ~ {Invisible:1b,Marker:1b,NoGravity:1b,Tags:["extinguish"]}
execute at @s run execute at @e[type=!armor_stand,type=!item,type=!item_frame,distance=1..5,tag=!summonedFireBase,tag=!Summoned] if block ~ ~ ~ minecraft:air run setblock ~ ~ ~ fire
