execute at @s run execute at @e[type=#enemy,distance=1..5,tag=!summonedFireBase,tag=!Summoned] if block ~ ~ ~ minecraft:air run summon armor_stand ~ ~ ~ {Invisible:1b,Marker:1b,NoGravity:1b,Tags:["extinguish"]}
execute at @s run execute at @e[type=#enemy,distance=1..5,tag=!summonedFireBase,tag=!Summoned] if block ~ ~ ~ minecraft:air run setblock ~ ~ ~ fire
execute at @s run execute as @e[type=#enemy,distance=1..5,tag=!summonedFireBase,tag=!Summoned] run data merge entity @s {Fire:320}
