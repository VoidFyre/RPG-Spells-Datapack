execute as @e[type=item,nbt={Item:{id:"minecraft:zombie_spawn_egg",tag:{Tags:["phylactery_item"]}}}] run function spells:spells/detectchunkloaders
...
execute as @a[gamemode=spectator] if score @s player_dead matches 1.. if score @s soul_stored matches 1 run function spells:spells/necro/entomb/resurrect
