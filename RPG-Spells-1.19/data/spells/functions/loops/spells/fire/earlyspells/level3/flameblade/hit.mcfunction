execute at @s run execute at @e[distance=1..5,tag=!summonedFireBase,tag=!Summoned] run setblock ~ ~ ~ fire
execute at @s run execute as @e[distance=1..5,tag=!summonedFireBase,tag=!Summoned] run data merge entity @s {Fire:320}
execute at @s run execute at @e[distance=1..5,tag=!summonedFireBase,tag=!Summoned] run summon armor_stand ~ ~ ~ {Invisible:1b,Marker:1b,NoGravity:1b,Tags:["extinguish"]}
