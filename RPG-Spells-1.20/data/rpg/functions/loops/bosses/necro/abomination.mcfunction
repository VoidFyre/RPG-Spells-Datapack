execute as @e[type=item,nbt={Item:{id:"minecraft:flint",tag:{abomination_spawner:1}}}] at @s run function rpg:loops/bosses/necro/summonabomination
...
execute as @e[type=zombie,tag=abomination] at @s if score @s abominationcooldown matches 0 run function rpg:loops/bosses/necro/abominationcast